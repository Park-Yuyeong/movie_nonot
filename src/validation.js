...
// detail 폼의 html id css코드 들과 재연결 필요함
    ...
    window.onload = function () {
        ...
        userName.onkeyup = function () {
            if (userName.value.length <= 2) { //userName의 글자수가 2보다 많은지 적은지에 따라 .hide 끄고 키기 
        ...
        userPassword.onkeyup = function () {
            ...
            if (userPassword.value.length === 4) { //userPassword 의 숫자의 갯수가 4일때만 유효성멘트가 꺼진다
        ...
        reviewInput.onkeyup = function () { //reviewInput의 값이 10 이상일때 유효성멘트가 꺼진다
        ...
        function activeEvent() {
            switch (!(userName.value && userPassword.value && reviewInput.value)) {
                case true: rvButton.disabled = true; break;
                case false: rvButton.disabled = false; break;
            }
        }
        function errorEvent() {
